...
export async function collectMetrics(){
 try{
  ...
  const metric={
   ...
   memory:Number(((mem.active/mem.total)*100).toFixed(2)),
   memory_used:mem.active,
   memory_total:mem.total,
   active_tasks:counts.active||0,
   queued_tasks:counts.queued||0,
   completed_tasks:counts.completed||0,
   failed_tasks:counts.failed||0,
  ...
  db.prepare(`
   INSERT INTO metrics(
    cpu,memory,memory_used,memory_total,
    active_tasks,queued_tasks,completed_tasks,
    failed_tasks,uptime,created_at
   ) VALUES(?,?,?,?,?,?,?,?,?,?)
  `).run(
   metric.cpu,
   metric.memory,
  ...
  logger.debug(
   ...
   {component:"metrics",operation:"collect"},
   metric
  );
